...
! function(t, e) {
	...
	draw: {
		toolbar: {
			...
			buttons: {
				polyline: 'Dibuje una linea sobre el mapa',
				polygon: 'Draw a polygon',
				rectangle: 'Draw a rectangle',
				circle: 'Draw a circle',
				marker: 'Dibuje un punto sobre el mapa'
			}
		},
		handlers: {
			circle: {
			...
			marker: {
				tooltip: {
					start: 'Haga click en el mapa para crear un punto'
				}
			},
			polygon: {
				tooltip: {
		...
		toolbar: {
			actions: {
				...
				cancel: {
					...
					text: 'Cancelar accion'
				}
			},
			buttons: {
				edit: 'Editar (mover el punto sobre el mapa)',
				editDisabled: 'No hay capas a editar',
				remove: 'Borrar punto',
				removeDisabled: 'No hay ningun punto a mover'
			}
		},
		handlers: {
			edit: {
				tooltip: {
					text: 'Seguidamente, seleccione "Guardar cambios" o "Cancelar accion"',
					subtext: 'Haga click sobre un punto y arrastre para moverlo en el mapa'
				}
			},
			remove: {
				tooltip: {
					text: 'Haga click sobre una entidad a eliminar',
					subtext: 'Seguidamente, seleccione "Guardar cambios" o "Cancelar accion"'
				}
			}
		}
	}
...
, L.Draw = {}, L.Draw.Feature = L.Handler.extend({
        ...
        _fireCreatedEvent: function() {
            ...
            {
                 line_distance.dist=this._getTooltipText().subtext.split(' ')[0];
        ...
        _onMouseMove: function(t) {
            ...
            this._tooltip.updatePosition(i), this._isDrawing && (this._drawShape(i), e = this._shape.getRadius().toFixed(1), this._tooltip.updateContent({
                text: this._endLabelText,
                subtext: o ? L.drawLocal.draw.handlers.circle.radius + ": " + L.GeometryUtil.readableDistance(e, a) : ""
            }))
        }
        ...
        _toggleMarkerHighlight: function() {
            if (this._icon) {
                var t = this._icon;
                t.style.display = "none", L.DomUtil.hasClass(t, "leaflet-edit-marker-selected") ? (L.DomUtil.removeClass(t, "leaflet-edit-marker-selected"), this._offsetMarker(t, -4)) : (L.DomUtil.addClass(t, "leaflet-edit-marker-selected"), this._offsetMarker(t, 4)), t.style.display = ""
            }
        },
        ...
        addHooks: function() {
            var t = this._poly;
            t instanceof L.Polygon || (t.options.editing.fill = !1), t.setStyle(t.options.editing), this._poly._map && (this._markerGroup || this._initMarkers(), this._poly._map.addLayer(this._markerGroup))
        },
        removeHooks: function() {
            var t = this._poly;
        ...
        addHooks: function() {
            var t = this._shape;
            t.setStyle(t.options.editing), t._map && (this._map = t._map, this._markerGroup || this._initMarkers(), this._map.addLayer(this._markerGroup))
        },
        removeHooks: function() {
            var t = this._shape;
...
//alert(this._getTooltipText().subtext.split(' ')[0])
    /*
  ...
  */
        ...
        _enableLayerEdit: function(t) {
            ...
            this._backupLayer(i), this.options.selectedPathOptions && (e = L.Util.extend({}, this.options.selectedPathOptions), e.maintainColor && (e.color = i.options.color, e.fillColor = i.options.fillColor), i.options.original = L.extend({}, i.options), i.options.editing = e), i.editing.enable()
        },
        _disableLayerEdit: function(t) {
            var e = t.layer || t.target || t;
            e.edited = !1, e.editing.disable(), delete e.options.editing, delete e.options.original
        },
        _onMouseMove: function(t) {
