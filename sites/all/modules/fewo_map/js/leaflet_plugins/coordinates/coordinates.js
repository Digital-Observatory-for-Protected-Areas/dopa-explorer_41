...
L.Control.Coordinates = L.Control.extend({
	...
	onAdd: function(map) {
		...
		this._utm_label = L.DomUtil.create("span", "labelFirst_utm", this._labelcontainer);
	...
	_update: function(evt) {
		...
		if (pos) {
			...
			var _lat_lng=[lat,lng];
		
			utm_array=proj4(wgs84,utm_19,_lat_lng);
		
			for (var p in utm_array)
			{
				 utm_y=(utm_array[p]).toString().split('.')[0];
			     utm_x=(utm_array[p]).toString().split('.')[0];
			}

			var utm_array2=[utm_y,utm_x];

			jQuery('.labelFirst_utm').text('UTM Y : '+utm_array2[0]+' UTM X : '+utm_array2[1]).css('display','block');
