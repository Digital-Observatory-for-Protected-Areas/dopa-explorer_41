    ...
    load: function (file /* File */) {
        ...
        if (fileSize > this.options.fileSizeLimit) {
            this.fire('data:error', {
                error: new Error('El fichero excede el tamaño permitido (' + fileSize + ' > ' + this.options.fileSizeLimit + 'kb)')
            });
            return;
        }
        ...
        if (!parser) {
            this.fire('data:error', {
                error: new Error('Formato de fichero no permitido ' + file.type + '(' + ext + ')')
            });
            return;
        }
...
L.Control.FileLayerLoad = L.Control.extend({
    statics: {
        TITLE: 'Cargar una capa local (GPX, KML, GeoJSON)',
        LABEL: '&#8965;'
    },
    options: {
        position: 'topleft',
