...
L.Control.Search = L.Control.extend({
	...
	options: {
		...
		zoom: 15,						//default zoom level for move to location
	...
	showLocation: function(latlng, title) {	//set location on map from _recordsCache
		...
		self._map.once('moveend zoomend', function(e) {
			if(self._markerLoc) {
				...
				else if ($('#ecofilter').parent().find('#searchtext27')[0].value.length > 0) {
					//simulating a click on the map fot the wdpa layer (use for open all the divs containg wdpa information when the user search of it)
					self._map.fireEvent('click',{latlng:_latlng})
				}
				else if ($('#locationfilter').parent().find('#searchtext27')[0].value.length > 0) {
					//simulating a click on the map fot the wdpa layer (use for open all the divs containg wdpa information when the user search of it)
					self._map.fireEvent('click',{latlng:_latlng})
				}
...
L.Control.Search.Marker = L.Marker.extend({
	...
	//TODO refact animate() more smooth! like this: http://goo.gl/DDlRs
		...
		var ecoid=$(selected_tip).attr('class').split(' ')[0];

		var countryid=$(selected_tip).attr('class').split(' ')[0];

		var filter_wdpa = "wdpaid='" + wdpaid + "'";

		var filter_eco = "ecoid='" + ecoid + "'";

		var filter_country = "iso_2digit='" + iso_2digit + "'";

			if ($('.mapwdpa').parent().find('#searchtext27')[0].value.length > 0) {

				map.options.wdpa_layer.setParams({CQL_FILTER:filter_wdpa});
			}
			else if ($('.mapecoregion').parent().find('#searchtext27')[0].value.length > 0) {

				map.options.eco_layer.setParams({CQL_FILTER:filter_eco});
			}
			else{
				map.options.layer_cou_cent.setParams({CQL_FILTER:filter_country});
			};
